import {
    ...
    EntryIssueLevelEnum, OffCyclePayRunTypeCode, PayRunCalculationStatusEnums, PayRunDetailedCalcStatus,
    PayRunMessageCodeEnum,
    PayRunPeriod, PayRunStatusEnum
} from "../../constant/constant.js";
import {getEmployeeIssues, getPayRunIssuesSummary, getWageTaxEmployeeIssues} from "./payrunIssues.js";

const getMockPayRuns =(startId = 0) =>{
    const mockPayRuns = [
        {
            payRunId: startId,
        ...
        },
        {
            ...
            payRunMessageTypes: [
                {
                    messageTypeId: 1,
                    shortName: 'Employee Calculation',
                    longName: 'Employee Calculation',
                    codeName: 'PAYROLLENGINE',
                },
                {
                    messageTypeId: 4,
                    shortName: 'Payroll Validation',
                    longName: 'My Pay',
                    codeName: 'MyPay',
                },
                {
                    messageTypeId: 5,
                    shortName: 'Quick Entry',
                    longName: 'Quick Entry',
                    codeName: 'PayrollQuickEntry',
                },
                {
                    messageTypeId: 6,
                    shortName: 'Checks',
                    longName: 'Checks',
                    codeName: 'PayrollChecks',
                },
                {
                    messageTypeId: 7,
                    shortName: 'Adjustments',
                    longName: 'Adjustments',
                    codeName: 'PayrollPriorPeriodAdjustments',
                },
                {
                    messageTypeId: 8,
                    shortName: 'Record of Employment',
                    longName: 'Record of Employment',
                    codeName: 'PayrollROEEntry',
                },
            ],
            payRunIssues: [
                {
    ...
    mockPayRuns.forEach(
        (payRun) => {
        const {payRunIssues} = payRun
        if (payRunIssues) {
            payRunIssues.forEach((issue) => {
...
export const mockPayRuns = Array.from(
    { length: 25 },
    (_value, i) =>
